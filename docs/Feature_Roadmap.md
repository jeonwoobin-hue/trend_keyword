...
| 콘텐츠 큐레이션 | CURATE-001, 002 | FR-CURATE-001 관련 콘텐츠 큐레이션, FR-CURATE-002 플랫폼별 필터, FR-CURATE-003 스크랩 | CURATE-001 목업 구현 (`pages/8_콘텐츠_큐레이션`, 커서 기반 "더 보기") — 플랫폼 필터는 멀티셀렉트가 `get_curated_contents()`에서 실제로 목록을 필터링, 스크랩은 `add_scrap`/`remove_scrap` + `pages/6_마이페이지` 목록 표시까지 구현(세션 저장만, 영속화 없음). CURATE-002(콘텐츠 상세)는 미착수 |
| 프로필/마이페이지 | MY-001, 002 | FR-PROFILE-001 프로필 조회, FR-PROFILE-002 관심사 수정, FR-PROFILE-003 알림 수신 설정, FR-PROFILE-004 회원 탈퇴 | 목업 구현 (`pages/6_마이페이지`, `pages/14_계정_설정`) — 알림 수신 설정은 세션 저장만, 회원 탈퇴는 `clear_user_data()`로 사용자 개인 데이터를 실제로 초기화 |
...
- `services/`의 `dashboard_service.py`, `alert_service.py`, `profile_service.py`, `auth_service.py`,
  ...
  모듈과 `app/session.py`가 `tests/services/`·`tests/app/`에 pytest 91건으로 커버되어
