...
- **계산식**: 전체 키워드 풀(카테고리 무관)에서 대상 키워드와 언급량 추이 시계열의 피어슨
  상관계수(`utils/statistics.pearson_correlation()`) 절댓값이 가장 큰 상위
  `RELATED_KEYWORDS_TOP_N`개(`services/dashboard_service.py`의 `_find_related_keywords()`)
- **한계(중요)**: 계산식 자체는 실제 산식이지만, 지금 입력값인 목 언급량 시계열은 키워드마다
  완전히 독립적인 난수로 생성되어 키워드 간 실제 의미적 연관성이 설계되어 있지 않다. 그 결과
  "오픈소스 LLM"↔"글래스 스킨 루틴"처럼 실제로는 무관한 키워드 쌍이 상관계수만 우연히 높게
  나와 연관 키워드로 뜰 수 있다 — 상관계수 계산 로직 자체의 결함이 아니라 입력 데이터의 한계다.
  실제 언급량 데이터(진짜 동시 언급 빈도)로 교체되면 같은 계산식이 의미 있는 결과를 낸다
- **미정 사항**: 실제 데이터 연동 시 "동시 언급"을 시계열 상관관계로 근사할지, 실제 동시 등장
  빈도(co-occurrence count)를 직접 셀지는 데이터 소스 확보 후 확정할 것
...
- **한계**: 계산식 자체는 실제 산식이지만, 입력값인 `dashboard_service`의 목 언급량 시계열이
  키워드마다 독립적인 난수라 실제 의미적 연관성이 반영되지 않는다(연관 키워드(DASH-002)와 같은
  한계 — 계산 로직 결함이 아니라 입력 데이터 한계). 실제 데이터 연동 시 계산식(피어슨 상관계수, 트리 구조 연결
  방식)은 그대로 재사용 가능
