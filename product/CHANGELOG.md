...
- **버그 픽스**: 이미 가입된 이메일로 회원가입을 시도하면 실제로는 아무 메일도 안 갔는데 화면엔
  "인증번호를 보냈습니다"가 뜨는 문제 발견(사용자 리포트). 원인: Supabase Auth는 이미 가입
  (확인)된 이메일로 재가입을 시도해도 에러를 던지지 않고 성공 응답을 준다 — 이메일 존재 여부가
  외부에 노출되지 않도록 하는 의도적인 보안 설계라 이메일도 실제로 발송하지 않음. `response.
  user.identities`가 빈 리스트인 것으로 이 상태를 구분해 `AuthError("VALID_003", "이미 가입된
  이메일입니다.")`로 명확히 안내하도록 `services/auth_service.request_signup_verification()` 수정.
  실제 Supabase로 재현·검증 완료.
- `tests/services/test_auth_service.py`에 케이스 추가, 기존 성공 테스트의 가짜 응답도 실제 응답
  형태(identities 포함)에 맞게 보정. 전체 테스트 129건 통과.
