...
|---|---|---|---|---|---|
| BL-001 | 소셜 로그인(구글/카카오) | P2 | FR-AUTH-002 | 완료 | `pages/0_로그인` — Supabase Auth OAuth(PKCE)로 실제 연동(2026-08-12). `services/auth_service.get_social_login_url()`/`complete_social_login()`. Google/Kakao Provider 설정과 Redirect URLs 등록은 대시보드에서 별도 진행 필요([ops/Deployment.md](../ops/Deployment.md)) |
| BL-002 | 비밀번호 재설정 | P2 | FR-AUTH-004 | 완료 | `pages/13_비밀번호_재설정` — Supabase Auth 이메일 OTP로 실제 연동(2026-08-12, Resend 커스텀 SMTP). 이메일 인증 → 새 비밀번호 설정 |
...
| BL-010 | 회원 탈퇴 | P2 | FR-PROFILE-004 | 완료 | `pages/14_계정_설정` — 확인 체크박스 + 탈퇴 버튼이 `services/auth_service.delete_account()`로 Supabase 계정을 실제 삭제(2026-08-12, `trendfit.profiles`는 cascade), 이후 `clear_user_data()`로 로컬 세션 초기화 |
